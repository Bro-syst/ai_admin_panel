...
import { isLocale, LOCALE_OPTIONS } from '@/core/i18n/locales'
...
export function SettingsPage() {
  ...
  return (
    <RolePageShell title={t('nav.settings')}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)]">
            ...
            <div className="mt-3 flex items-center justify-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2">
              <span className={['text-sm font-semibold', mode === 'light' ? 'text-[var(--text)]' : 'text-[var(--text-muted)]'].join(' ')}>
              ...
              <span className={['text-sm font-semibold', mode === 'dark' ? 'text-[var(--text)]' : 'text-[var(--text-muted)]'].join(' ')}>
                {t('common.theme_dark')}
              </span>
            </div>
          </section>
