# Prompt 05: Urgent Release Evidence Requirements UI Acceptance Review
...
```text
Prompt '05_urgent_release_evidence_requirements_ui_acceptance_review_prompt.md'
...
2. Продолжай только если:
   - `allowed_next_step = 05_urgent_release_evidence_requirements_ui_acceptance_review`;
...
2. Scope fit:
   ...
   - no release evidence UI/state moved to another module;
   - accepted Stages 01-13 not degraded.
3. Release requirements contract:
   - new endpoint is loaded;
   - fields are mapped in API/model boundary;
   - unknown backend values are preserved safely;
   - backend remains release gate authority.
4. Evidence UI/runtime:
   - release setup blockers and evidence status render;
   - smoke-case matrix comes from backend `required_smoke_cases`;
   - templates with empty smoke cases do not show a hardcoded matrix;
   - stable reference guidance uses backend metadata;
   - create is blocked without complete evidence or allowed manual override;
...
5. Payload correctness:
   ...
   - all `evidence.smoke_cases[]` are submitted;
   - manual override payload includes reason code, related items and comment
     when used;
   - localized labels are not submitted as backend values.
6. Evidence/security:
   - mutation evidence remains visible after actions;
   - missing optional fields use neutral copy;
...
10. If findings exist:
   ...
   - set `allowed_next_step = 04_urgent_release_evidence_requirements_ui_implementation_fix_pass`.
11. If no findings:
   - update PIPELINE_STATE_PATH with urgent stage acceptance;
   - set `allowed_next_step = 06_urgent_release_evidence_requirements_ui_docs_sync`.
