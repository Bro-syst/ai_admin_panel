...
This document fixes the development rules for `apps/admin-panel`. The app is a clean start for an internal AI administration frontend, so every change should strengthen clear module boundaries instead of turning the app into one large shared surface.
