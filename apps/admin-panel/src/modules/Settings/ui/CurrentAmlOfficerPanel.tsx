...
function resolveRoleLabel(role: string, t: (key: string) => string) {
  const translated = t(`aml.roles.${role}`)
  return translated === `aml.roles.${role}` ? role : translated
}
...
export function CurrentAmlOfficerPanel({
  amlOfficer,
  authState,
}: {
  ...
  if (!amlOfficer || !authState) return null
        ...
        <article className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
          ...
          <dl className="mt-4 space-y-4">
            <div>
              ...
              <dd className="mt-1 break-all text-sm font-medium text-[var(--text)]">{amlOfficer.email}</dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t('common.role')}</dt>
              <dd className="mt-1 text-sm font-medium text-[var(--text)]">{resolveRoleLabel(amlOfficer.role, t)}</dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t('common.status')}</dt>
              <dd className="mt-1 text-sm font-medium text-[var(--text)]">{resolveStatusLabel(amlOfficer.status, t)}</dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t('settings.account.last_login')}</dt>
              <dd className="mt-1 text-sm font-medium text-[var(--text)]">{formatDateTime(amlOfficer.lastLoginAt)}</dd>
            </div>
          </dl>
        </article>
