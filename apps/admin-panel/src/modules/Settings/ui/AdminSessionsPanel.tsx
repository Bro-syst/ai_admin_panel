...
type PendingConfirmation =
  | { kind: 'logout_all' }
  | { kind: 'revoke'; session: AdminAuthSessionItem }
  | null
...
export function AdminSessionsPanel() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      setError(readAuthErrorMessage(nextError, t('settings.sessions.load_error'), t))
  ...
  async function handleLogoutAll() {
    ...
    try {
      ...
      setError(readAuthErrorMessage(nextError, t('settings.sessions.logout_all_error'), t))
      setErrorCause(nextError)
    } finally {
  ...
  async function handleRevoke(session: AdminAuthSessionItem) {
    ...
    try {
      ...
      setError(readAuthErrorMessage(nextError, t('settings.sessions.revoke_error'), t))
      setErrorCause(nextError)
    } finally {
      setPendingAction(null)
    }
    ...
    <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] md:col-span-2">
      ...
      {status === 'error' ? (
        <div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 text-center">
          ...
          <RefreshButton className="mt-4" onClick={() => void loadSessions()} />
        </div>
      ) : null}
