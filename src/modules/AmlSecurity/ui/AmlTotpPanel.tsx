...
import { useAmlTotpManager } from '@/modules/AmlSecurity/model/useAmlTotpManager'
...
export function AmlTotpPanel() {
  ...
  return (
    <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] md:col-span-2">
      ...
      {cooldownMessage ? (
        <div className="mt-4 rounded-2xl border border-amber-300 bg-amber-500/10 px-4 py-3 text-sm font-medium text-amber-700 dark:border-amber-500/25 dark:bg-amber-500/15 dark:text-amber-300">
          <div>{cooldownMessage}</div>
          <ErrorSupportDetails error={cooldownCause} className="mt-3" />
        </div>
      ) : null}
                    ...
                    <dl className="grid min-w-0 gap-3">
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
                        <dt className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">
                          {t('settings.totp.provisioning_manual_entry_key')}
                        </dt>
